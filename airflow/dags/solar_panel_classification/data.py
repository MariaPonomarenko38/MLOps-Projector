from tqdm import tqdm
import cv2      
from keras.applications.vgg16 import VGG16
from keras.applications import MobileNet
import numpy as np
import os
from dotenv import load_dotenv
import json
from sklearn.utils import shuffle
from solar_panel_classification.utils import load_h5, save_in_h5, get_args 
from solar_panel_classification.minio_client import MinioClient
from airflow.models import Variable
from pathlib import Path

load_dotenv()

def preprocessing(minio_client, bucket_name, object, image_size):
    image_data = minio_client.client.get_object(bucket_name, object.object_name).data
    image_array = np.frombuffer(image_data, np.uint8)
    image = cv2.imdecode(image_array, cv2.IMREAD_COLOR)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    image = cv2.resize(image, image_size)
    return image

def load_inference_data(path_to_args: Path, path_to_interim_data, bucket_name):
    args = get_args(path_to_args)
    minio_client = MinioClient(Variable.get("access_key"), Variable.get("secret_key"),"172.17.0.1:9000/")
    objects, last_folder_name = minio_client.get_data(bucket_name)
    images = []
    for obj in objects:
        image = preprocessing(minio_client, bucket_name, obj, tuple(args['image_size']))
        images.append(image)
    images = np.array(images, dtype = 'float32')
    images = images / 255.0
    save_in_h5(path_to_interim_data + last_folder_name + "_images.h5", images)
    return last_folder_name

def get_features(args, images):
    model = MobileNet(input_shape=tuple(args["mobile_net_input_shape"]), weights='imagenet', include_top=False)
    features = model.predict(images)
    return features

def process_data(path_to_args: Path,interim_path_data, processed_path_data, **context):
    args = get_args(path_to_args)
    last_folder_name = context['task_instance'].xcom_pull(task_ids='load_data')
    images = load_h5(interim_path_data + f'{last_folder_name}_images.h5')

    features = get_features(args, images)

    save_in_h5(processed_path_data + f'{last_folder_name}_features.h5', features)

def upload_data(bucket_name, file_name, file_path, **context):
    minio_client = MinioClient(Variable.get("access_key"), Variable.get("secret_key"),"172.17.0.1:9000/")
    minio_client.upload_file(file_name, bucket_name, file_path, context['task_instance'].xcom_pull(task_ids='load_data'))